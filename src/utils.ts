import { CollectionConfig, OutputItem } from './types';
  ...
  try {
    ...
    switch (displayFormat) {
      ...
      case 'formatted':
        ...
        return typeof parsed === 'object' && parsed.value ? parsed.value : typeof parsed === 'object' && parsed.language ? parsed.language : typeof parsed === 'object' && parsed.name ? parsed.name : typeof parsed === 'object' && parsed.title ? parsed.title : typeof parsed === 'object' ? JSON.stringify(parsed) : String(parsed);
      ...
      case 'related-values':
        ...
        return typeof parsed === 'object' ? `${parsed.collection || ''}: ${parsed.value || parsed.language || parsed.title || parsed.name || parsed.id || ''}` : String(parsed);
      ...
      case 'labels':
        ...
        return typeof parsed === 'object' ? (parsed.language || parsed.title || parsed.name || parsed.value || parsed.id || JSON.stringify(parsed)) : String(parsed);
...
export const createOutputItem = (item: any, config: CollectionConfig, index: number): OutputItem => {
...
export function getLimitKey(useSharedLimit: boolean, collection?: string): string {
  return useSharedLimit ? 'm2a-limit-selector-shared-limit' : `m2a-limit-selector-limit-${collection}`;
}
